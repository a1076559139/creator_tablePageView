...
cc.Class({
    ...
    properties: {
        ...
        pageChangeEvents: {
            ...
            tooltip: '翻页时，触发回调，向回调传入两个参数，参数一为当前处于哪一页，参数二为一共多少页',
        },
    ...
    _initCell: function (cell) {
        if (this.Type == Type.GRID) {
            if (this.viewType == viewType.tableView || (this._scrollView.horizontal && this.Direction == Direction.TOP_TO_BOTTOM__LEFT_TO_RIGHT) || (this._scrollView.vertical && this.Direction == Direction.LEFT_TO_RIGHT__TOP_TO_BOTTOM)) {
        ...
        if (this._cellPool.size() == 0) {
            if (this.Type == Type.GRID) {
                ...
                if (this._scrollView.horizontal) {
                    ...
                    // node.height = cell.height * this._groupCellCount + (this._groupCellCount - 1) * this.Spacing + 2 * this.Padding;
                    ...
                    // node.width = cell.width * this._groupCellCount + (this._groupCellCount - 1) * this.Spacing + 2 * this.Padding;
    ...
    initTableView: function (count, data) {
        ...
        if (!this._initSuccess) {
            ...
            // this._scrollView.node.on('touchstart', function () { });
    ...
    _pageMove: function (event) {
        ...
        if (this.viewType == viewType.pageView) {
            if (this._scrollView.horizontal) {
                y = 0;
                if (Math.abs(event.getLocation().x - event.getStartLocation().x) > 7) {
                ...
                x = 0;
                if (Math.abs(event.getLocation().y - event.getStartLocation().y) > 7) {
